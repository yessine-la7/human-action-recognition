...
class ActionRecognitionDataset(Dataset):
    def __init__(self, csv_file, root_dir, transform=None, is_test=False):
        ...
        if not self.is_test:
            ...
            self.data_frame["encoded_label"] = self.label_encoder.fit_transform(self.data_frame["label"])
        else:
    ...
    def __getitem__(self, idx):
        ...
        img_name = os.path.join("..", "Human Action Recognition","train", self.data_frame.iloc[idx, 0])
        ...
        if not self.is_test:
            ...
            # Return the transformed image, original image, and label
            return image, original_image, torch.tensor(label, dtype=torch.long)
        else:
